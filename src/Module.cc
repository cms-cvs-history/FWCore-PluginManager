...
//#//include "FWCore/PluginManager/interface/ShellEnvironment.h"
...
const char Module::s_modEntryPoint [] = "EDM_MODULE";
      ...
      m_definition (0)
{
    LOG (0, trace, LFplugin_manager,
	 "constructing module for library `" << m_libraryName.string() << "'\n");
    makeCache ();
}
...
{
    LOG (0, trace, LFplugin_manager,
         "destroying module for library `" << m_libraryName.string() << "'\n");
...
Module::library (void) const
{
    ...
    {
	LOG (0, trace, LFplugin_manager,
	     "loading shared library `" << m_libraryName.string() << "'\n");
	...
	try
	{
	    ...
	    m_library = SharedLibrary::load ( m_libraryName.string());
	}
	catch (cms::Exception &e)
	{
	    m_manager->feedback (PluginManager::ErrorLoadFailure, m_libraryName, &e);
	    throw PluginError ("SharedLibraryLoad",e);
	}
    }
...
Module::load (void)
{
    ...
    LOG (0, trace, LFplugin_manager, "loading `" << m_libraryName.string() << "'\n");
...
Module::unload (void)
{
    ...
    if (m_library)
    {
        LOG (0, trace, LFplugin_manager, "unloading `" << m_libraryName.string() << "'\n");

	if (NULL == getenv ("SEAL_KEEP_MODULES") )
	{
...
{ m_cache = new ModuleDescriptor (0, s_tag, m_libraryName.string()); }
...
Module::checkEntryPoints (void) const
{
	...
	throw PluginError ("CheckEntryPoint",e);
    }
}
...
Module::attach (void)
{
	...
	LOG (0, trace, LFplugin_manager,
	     "attaching module " << libraryName().string() << '\n' << indent);
...
Module::detach (void)
{
    ...
    {
	LOG (0, trace, LFplugin_manager,
	     "detaching module " << libraryName ().string() << '\n' << indent);
...
Module::query (void)
{
	...
	LOG (0, trace, LFplugin_manager,
	     "querying module " << libraryName ().string() << '\n' << indent);
